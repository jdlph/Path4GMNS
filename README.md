...
### Dependency
Path4GMNS is written in **Python 3.x**, which is the minimum requirement to use the basic functionalities of Path4GMNS. Some of its modules require futher run-time supports. We will go through them one by one along with the use case in the following section
...
The current implemenation supprts evaluations for the following three modes. More modes will be added in the future to accommodate the full set of allowed uses for links as specified by GMNS.
